package com.app.styletap.webtoappconverter.presentations.utils

object Contants {

    const val ACTION_FINISH_ACTIVITY = "ACTION_FINISH_ACTIVITY"
    const val ACTION_REFRESH_ACTIVITY = "ACTION_REFRESH_ACTIVITY"

    const val ERROR = "ERROR"
    const val DRAFT = "DRAFT"
    const val PROCESSING = "PROCESSING"
    const val READY_TO_DOWNLOAD = "READY_TO_DOWNLOAD"
    const val DRAFT_BUNDLE = "DRAFT_BUNDLE"
    const val PROCESSING_BUNDLE = "PROCESSING_BUNDLE"
    const val READY_TO_DOWNLOAD_BUNDLE = "READY_TO_DOWNLOAD_BUNDLE"

    const val PLAY_STORE = "PLAY_STORE"
    const val CUSTOM_APP = "CUSTOM_APP"
    const val PUSH_NOTIFICATIONS = "PUSH_NOTIFICATIONS"
    const val ADMOB_MONETIZATION = "AdMob Monetization"
    const val PREMIUM_BUILD = "PREMIUM_BUILD"

    const val CREATE_APP = "CREATE_APP"
    const val CREATE_BUNDLE = "CREATE_BUNDLE"
    const val UPLOAD_APP = "UPLOAD_APP"


    const val isShowOnBoarding = "isShowOnBoarding"
    const val languageCode = "languageCode"
    const val isLanguageSelected = "isLanguageSelected"

}